...
- **Icons are local components** (`src/lib/icons/`, generated by `pnpm icons` from the Iconify API) instead of `unplugin-icons` over `@iconify/json` — which weighed 437 MB of the 643 MB `node_modules`, for seventeen icons. Same markup, same Carbon and Akar sets; three dependencies and a Vite plugin gone
- **Two unused dependencies dropped**: `@sveltejs/adapter-auto` (the site builds with adapter-static) and `dotenv` — `build.js` loaded a `.env` that nothing reads, and `.env.example` said as much. The `svelte-i18n` esbuild override now follows Vite's line, so one esbuild binary is installed instead of two
