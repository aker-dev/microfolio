...
- Deployment to GitHub Pages is triggered by a push to **`main`** — `.github/workflows/deploy.yml` (it used to be a separate `preview` branch until 0.13.2; `main` is simpler for forks and clones, which publish with a plain push). Work happens on `dev`; publishing means merging `dev` into `main`, and a release tags that merge
