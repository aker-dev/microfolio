...
- The zip holds the thirty-project "Bauhaus" demo set, produced by `pnpm generate-demo` (`scripts/generate-demo-projects.js` — maintainer tool, shells out to `zip`/`unzip`). The dataset is handwritten in `scripts/demo-projects/projects.js`; the images are seeded geometric compositions (`compose.js`), so regeneration is byte-identical until the dataset changes. Every project carries the `bauhaus` tag because the e2e pagination specs filter on one tag and need 21+ matches; eleven are `featured` so the homepage grid holds twelve with `example-project`. `--install` refreshes the unzipped working copies (it deletes the gitignored project directories and re-extracts)
