...
- That workflow unzips `content/projects/example_projects.zip` before installing, so the demo content CI builds and tests against comes from the zip, not from the working copy. **`.gitignore` ignores the demo by name** — a block between two marker comments that `pnpm generate-demo` rewrites from the dataset — and never `content/projects/*`: a user's projects must reach their repository, and until 0.13.2 they did not, so a site that followed the docs was published by GitHub Pages without a single one of them. It then runs lint, unit tests and e2e before the build
